...
class ChartContentRepositoryRemote @Inject constructor(
    ...
    override suspend fun getSorted(sortBy: Any, limit: Int?, offset: Int): Flow<Result<List<Chart>>> {
        val option = sortBy as? SortOption ?: SortOption.LAST_UPDATED
        return chartRepository.getChartsSortedBy(option, limit, offset)
    }

    override suspend fun search(query: String, limit: Int?, offset: Int): Flow<Result<List<Chart>>> {
    ...
    override suspend fun update(items: List<Chart>): Flow<Result<Boolean>> =
        flow { emit(Result.success(true)) }

    override suspend fun delete(items: List<Chart>): Flow<Result<Boolean>> =
        flow { emit(Result.success(true)) }
}
